...
int input_buffer_validate (struct input_buffer *buffer)
{
	...
	{
		/* Buffer is corrupted */
		fprintf(stderr, "[input_buffer.c]{validate}: Buffer CORRUPT: start:%p border:%p end:%p current:%p size:%ld front:%ld back:%ld diff:%ld avail:%d write:%d\n", buffer->start, buffer->border, buffer->end, buffer->current, buffer->end - buffer->start, buffer->border - buffer->start, buffer->end - buffer->border, buffer->current - buffer->border, buffer->available, buffer->write);
		return FALSE;
	}
	
	/* Buffer is in good health */
	fprintf(stderr, "[input_buffer.c]{validate}: Buffer CORRECT: start:%p border:%p end:%p current:%p size:%ld front:%ld back:%ld diff:%ld avail:%d write:%d\n", buffer->start, buffer->border, buffer->end, buffer->current, buffer->end - buffer->start, buffer->border - buffer->start, buffer->end - buffer->border, buffer->current - buffer->border, buffer->available, buffer->write);
